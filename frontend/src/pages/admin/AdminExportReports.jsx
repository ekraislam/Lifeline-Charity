    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {reports.map(report => (
                    <div key={report.key} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                        <div className={`bg-gradient-to-r ${report.color} p-6 text-white`}>
                            <div className="text-4xl mb-2">{report.icon}</div>
                            <h3 className="text-lg font-bold">{report.title}</h3>
                        </div>
                        <div className="p-5">
                            <p className="text-sm text-gray-600 mb-4 min-h-[48px]">{report.description}</p>
