    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full">
                <h3 className="text-lg font-bold text-gray-900 mb-4">Edit Campaign</h3>
                                ...
                                {campaigns.map(c => (
                                    <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-3">
                                            <p className="text-sm font-semibold text-gray-900 max-w-[180px] truncate" title={c.title}>{c.title}</p>
                                        </td>
                                        <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{c.ngo_name || c.ngo_user_name || '—'}</td>
                                        <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{c.category_name || '—'}</td>
                                        <td className="px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">${parseFloat(c.goal_amount||0).toLocaleString()}</td>
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${STATUS_COLORS[c.status]||'bg-gray-100 text-gray-800'}`}>
                                                {c.status}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{new Date(c.created_at).toLocaleDateString()}</td>
                                        <td className="px-4 py-3">
