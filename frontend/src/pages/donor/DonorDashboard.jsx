    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">{t('dashboard.welcome')}, {user?.name}!</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('donor.title')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xs p-6 border-t-4 border-primary-500 border border-gray-100 dark:border-gray-700">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{t('campaigns.title')}</h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">{t('campaigns.subtitle')}</p>
                    <Link to="/campaigns" className="text-primary-600 dark:text-primary-400 text-xs font-extrabold hover:underline">{t('home.exploreCampaigns')} &rarr;</Link>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xs p-6 border-t-4 border-emerald-500 border border-gray-100 dark:border-gray-700">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{t('donor.donationHistory')}</h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">{t('donor.downloadReceipt')}</p>
                    <Link to="/donations/history" className="text-emerald-600 dark:text-emerald-400 text-xs font-extrabold hover:underline">{t('dashboard.viewAll')} &rarr;</Link>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xs p-6 border-t-4 border-amber-500 border border-gray-100 dark:border-gray-700">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{t('profile.title')}</h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">{t('profile.personalInfo')}</p>
                    <Link to="/profile" className="text-amber-600 dark:text-amber-400 text-xs font-extrabold hover:underline">{t('profile.saveChanges')} &rarr;</Link>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-md border border-gray-100 dark:border-gray-700 space-y-4">
                <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 pb-4">
                    <div>
                        <h2 className="text-xl font-extrabold text-gray-900 dark:text-white flex items-center gap-2">
                            <span>🎁</span> {t('donor.donationHistory')}
                        </h2>
                ...
                {loading ? (
                    ...
                    <div className="p-8 text-center text-gray-400 font-medium">{t('donor.noDonations')}</div>
                ) : (
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            <thead className="bg-gray-50 dark:bg-gray-900 text-xs font-bold uppercase text-gray-500">
                                <tr>
                                    ...
                                    <th className="py-3.5 px-4 text-left">{t('donor.table.status')}</th>
                                    <th className="py-3.5 px-4 text-left">{t('donor.table.date')}</th>
                                </tr>
                            </thead>
