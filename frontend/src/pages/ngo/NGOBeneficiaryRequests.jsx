...
const NGOBeneficiaryRequests = () => {
    ...
    if (loading) return <div className="p-12 text-center text-gray-500 dark:text-gray-400">Loading waiting requests...</div>;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {confirmId && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 max-w-md w-full">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Confirm Accept</h3>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setConfirmId(null)} className="flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl hover:bg-gray-200 dark:bg-gray-700 font-medium text-sm">
                                Cancel
                            </button>
                            <button
            ...
            <div className="mb-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Beneficiary Help Requests</h1>
                    ...
                    <button onClick={() => navigate('/ngo/dashboard')} className="px-4 py-2 text-sm border rounded-lg hover:bg-gray-50 dark:bg-gray-900">
                        ← Back to Dashboard
                    </button>
                </div>
            </div>
            ...
            {requests.length === 0 ? (
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-12 text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 font-medium text-lg">No waiting requests at this time</p>
                    <p className="text-gray-400 text-sm mt-1">All verified beneficiaries have been assigned to NGOs</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {requests.map(req => (
                        <div key={req.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                            <div className="p-5 flex-grow">
                                ...
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2 line-clamp-2">{req.title}</h3>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 line-clamp-3">{req.description}</p>

                                <div className="space-y-2 text-sm">
                                    <div className="flex items-center justify-between">
                                        <span className="text-gray-500 dark:text-gray-400">Beneficiary</span>
                                        <span className="font-semibold text-gray-900 dark:text-white">{req.beneficiary_name}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-gray-500 dark:text-gray-400">Required Amount</span>
                                        <span className="font-bold text-green-700">${parseFloat(req.required_amount || 0).toLocaleString()}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-gray-500 dark:text-gray-400">Submitted</span>
                                        <span className="text-gray-700 dark:text-gray-200">{new Date(req.created_at).toLocaleDateString()}</span>
                                    </div>
                                </div>
                            </div>

                            <div className="p-5 border-t bg-gray-50 dark:bg-gray-900">
                                <button
